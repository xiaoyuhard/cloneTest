...
* 模块名: OneFlyGUITools.cs
* 创建者：RyuRae
* 修改者列表：
* 创建日期：
* 功能描述：
***********************************************/
public class OneFlyGUITools {
    ...
    public static bool BeginToogleGroup(string text, bool value, int indent = 0)
    {
        ...
        if (value)
            OneFlyGUITools.BeginGroup(indent);
    ...
    {
        if (value)
            OneFlyGUITools.EndGroup(space);

        EditorGUILayout.EndToggleGroup();
    }

    public static bool Button(string label, Color color, int width, bool leftAligment = false, int height = 0)
    {
    ...
    public static void DrawTextureRectPreview(Rect rect, Rect textureRect, Texture2D tex, Color color)
    {
        ...
        DrawTileTexture(rect, OneFlyGUITools.GetCheckerTexture());
    ...
    {
        if (OneFlyGUITools.checkerTexture == null)
        {
            OneFlyGUITools.checkerTexture = CreateCheckerTexture();
        }
